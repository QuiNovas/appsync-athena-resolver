...
def handler(event, context):
    ...
    if type(event) is dict:
        ...
        with _get_connection(event.get('schemaName')).cursor(max_workers=1) as cursor:
            _, future = cursor.execute(event['query'].format(**event.get('params', {})))
        ...
        with _get_connection(event.get('schemaName')).cursor(max_workers=max_workers) as cursor:
            futures = []
            for item in event:
